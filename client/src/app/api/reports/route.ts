  ...
  for (let attempt = 1; attempt <= retries + 1; attempt++) {
    try {
      return await new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => reject(new Error("UploadTimeout")), timeoutMs);
...
function validateDescription(desc: string): { isValid: boolean; error?: string } {
  const t = desc.trim();
...
export async function POST(req: Request) {
  try {
    ...
    if (!descVal.isValid) return NextResponse.json({ error: descVal.error }, { status: 400 });
    if (!validateCategory(category)) return NextResponse.json({ error: "Invalid category" }, { status: 400 });
    ...
    if (file) {
      ...
      if (file.size > MAX_SIZE) {
        return NextResponse.json({ error: `File too large (> ${MAX_SIZE / 1024 / 1024}MB)` }, { status: 413 });
      }
