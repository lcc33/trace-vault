...
export default function ProfilePage() {
  ...
  if (!session) {
    return (
      <div className="">
        Please <Link href={"/login"}>sign in</Link> to view your profile
      </div>
    );
  }
            ...
            {loading ? (
              ...
              <div className="space-y-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {userReports.map((report) => (
                  ...
                  >
                    {report.imageUrl &&
                    report.imageUrl !== "/placeholder-image.png" ? (
                    ...
                    <p className="flex-1 text-base mb-2 text-slate-100">
                      {report.description}
                    </p>
